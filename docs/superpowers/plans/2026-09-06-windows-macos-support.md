...
- [x] Automated verification: run all Node tests and portable PowerShell suites on macOS; run Windows-specific PowerShell tests in Windows CI. Add a Windows/macOS CI matrix, syntax and repository hygiene checks. Both platforms passed on `3c08457`; subsequent fixes must pass the same checks before delivery.
