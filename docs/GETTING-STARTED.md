...
双击桌面的 **Codex Discord 控制台**。两平台均显示服务、自启、Discord、Codex 桌面端、继续队列和最后活动状态，每两秒刷新。关闭控制台窗口后，桥接仍在后台运行。
...
先处理旧机队列并停止旧桥接，再开启新机桥接，避免两个实例同时消费同一个 Bot 的消息。迁移包不包含 Codex 登录、任务数据库、项目文件、待执行回复或旧任务映射；这些不会因为导入 Bot 设置而出现在新电脑。
